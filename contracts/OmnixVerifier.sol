...
contract OmnixVerifier {
    ...
    function verifyTransfer(Utils.G1Point[] memory CLn, Utils.G1Point[] memory CRn, Utils.G1Point[] memory C, Utils.G1Point memory D, Utils.G1Point[] memory y, uint256 epoch, Utils.G1Point memory u, bytes memory proof) public view returns (bool) {
        OmnixStatement memory statement;
        ...
        statement.u = u;
        OmnixProof memory omnixProof = unserialize(proof);
        return verify(statement, omnixProof);
    }

    struct OmnixAuxiliaries {
        uint256 y;
        uint256[64] ys;
        uint256 z;
    ...
    function verify(OmnixStatement memory statement, OmnixProof memory proof) internal view returns (bool) {
        ...
        OmnixAuxiliaries memory omnixAuxiliaries;
        omnixAuxiliaries.y = uint256(keccak256(abi.encode(anonAuxiliaries.w))).mod();
        omnixAuxiliaries.ys[0] = 1;
        omnixAuxiliaries.k = 1;
        for (uint256 i = 1; i < 64; i++) {
            omnixAuxiliaries.ys[i] = omnixAuxiliaries.ys[i - 1].mul(omnixAuxiliaries.y);
            omnixAuxiliaries.k = omnixAuxiliaries.k.add(omnixAuxiliaries.ys[i]);
        }
        omnixAuxiliaries.z = uint256(keccak256(abi.encode(omnixAuxiliaries.y))).mod();
        omnixAuxiliaries.zs[0] = omnixAuxiliaries.z.mul(omnixAuxiliaries.z);
        omnixAuxiliaries.zs[1] = omnixAuxiliaries.zs[0].mul(omnixAuxiliaries.z);
        omnixAuxiliaries.zSum = omnixAuxiliaries.zs[0].add(omnixAuxiliaries.zs[1]).mul(omnixAuxiliaries.z);
        omnixAuxiliaries.k = omnixAuxiliaries.k.mul(omnixAuxiliaries.z.sub(omnixAuxiliaries.zs[0])).sub(omnixAuxiliaries.zSum.mul(1 << 32).sub(omnixAuxiliaries.zSum));
        omnixAuxiliaries.t = proof.tHat.sub(omnixAuxiliaries.k); // t = tHat - delta(y, z)
        for (uint256 i = 0; i < 32; i++) {
            omnixAuxiliaries.twoTimesZSquared[i] = omnixAuxiliaries.zs[0].mul(1 << i);
            omnixAuxiliaries.twoTimesZSquared[i + 32] = omnixAuxiliaries.zs[1].mul(1 << i);
        }

        omnixAuxiliaries.x = uint256(keccak256(abi.encode(omnixAuxiliaries.z, proof.T_1, proof.T_2))).mod();
        omnixAuxiliaries.tEval = proof.T_1.mul(omnixAuxiliaries.x).add(proof.T_2.mul(omnixAuxiliaries.x.mul(omnixAuxiliaries.x))); // replace with "commit"?
        ...
        sigmaAuxiliaries.A_b = Utils.g().mul(proof.s_b).add(anonAuxiliaries.DR.mul(omnixAuxiliaries.zs[0].neg()).add(anonAuxiliaries.CRnR.mul(omnixAuxiliaries.zs[1])).mul(proof.s_sk).add(anonAuxiliaries.CR[0][0].add(Utils.g().mul(fee.mul(anonAuxiliaries.wPow))).mul(omnixAuxiliaries.zs[0].neg()).add(anonAuxiliaries.CLnR.mul(omnixAuxiliaries.zs[1])).mul(proof.c.neg())));
        sigmaAuxiliaries.A_X = anonAuxiliaries.y_XR.mul(proof.s_r).add(anonAuxiliaries.C_XR.mul(proof.c.neg()));
        sigmaAuxiliaries.A_t = Utils.g().mul(omnixAuxiliaries.t).add(omnixAuxiliaries.tEval.neg()).mul(proof.c.mul(anonAuxiliaries.wPow)).add(Utils.h().mul(proof.s_tau)).add(Utils.g().mul(proof.s_b.neg()));
        sigmaAuxiliaries.gEpoch = Utils.mapInto("Omnix", statement.epoch);
        sigmaAuxiliaries.A_u = sigmaAuxiliaries.gEpoch.mul(proof.s_sk).add(statement.u.mul(proof.c.neg()));

        sigmaAuxiliaries.c = uint256(keccak256(abi.encode(omnixAuxiliaries.x, sigmaAuxiliaries.A_y, sigmaAuxiliaries.A_D, sigmaAuxiliaries.A_b, sigmaAuxiliaries.A_X, sigmaAuxiliaries.A_t, sigmaAuxiliaries.A_u))).mod();
        ...
        for (uint256 i = 0; i < 64; i++) {
            ipAuxiliaries.hPrimes[i] = ip.hs(i).mul(omnixAuxiliaries.ys[i].inv());
            ipAuxiliaries.hPrimeSum = ipAuxiliaries.hPrimeSum.add(ipAuxiliaries.hPrimes[i].mul(omnixAuxiliaries.ys[i].mul(omnixAuxiliaries.z).add(omnixAuxiliaries.twoTimesZSquared[i])));
        }
        ipAuxiliaries.P = proof.BA.add(proof.BS.mul(omnixAuxiliaries.x)).add(gSum().mul(omnixAuxiliaries.z.neg())).add(ipAuxiliaries.hPrimeSum);
    ...
    function unserialize(bytes memory arr) internal pure returns (OmnixProof memory proof) {
